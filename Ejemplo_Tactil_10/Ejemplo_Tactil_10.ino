...
{ 
 ...
 tft.drawRect(40, 100, 160, 60, RED); // Dibujamos un "boton"
  
  tft.setCursor(50,115);  // Colocamos el cursor
  tft.setTextSize(4);    // Especificamos el tamaño del texto
  tft.setTextColor(RED); // Definimos el color del texto 
...
{          
 ...
 if((X > 40 && X < 200) && (Y > 100 && Y < 160) && (Z > MINPRESSURE && Z < MAXPRESSURE)) 
    {
      if (b == 0) // Si la variable flag esta en 0
      {
        tft.fillRect(40, 100, 160, 60, RED); // Dibujamos nuestro boton relleno de color
        
        tft.setCursor(70,105); // Colocamos el cursor
        tft.setTextSize(3);   // Especificamos el tamaño del texto
        tft.setTextColor(WHITE); // Definimos el color del texto 
        tft.println("BOTON2"); // Escribimos por pantalla
        tft.setCursor(60,130);  // Colocamos el cursor
        tft.println("PULSADO"); // Escribimos por pantalla   
      ...
      {
         tft.fillRect(41, 101, 158, 58, CYAN); // Dibujamos el fondo de nuestro boton en cyan
        ...
        digitalWrite(ledK, LOW);
         ...
         tft.setCursor(15,170); // Colocamos el cursor
         tft.setTextSize(2); // Especificamos el tamaño del texto
         tft.setTextColor(RED); // Definimos el color del texto 
         tft.println("TEMP.: "); // Escribimos por pantalla


       
         
         
         tft.setCursor(15,190); // Colocamos el cursor
         tft.setTextSize(2); // Especificamos el tamaño del texto
         tft.setTextColor(RED); // Definimos el color del texto 
         tft.println("HUM.: "); // Escribimos por pantalla
...

{
 ...
 tft.fillRect(90, 170, 110, 18, RED);

tft.fillRect(90, 190, 110, 18, RED);
 
tft.setCursor(90,170); // Colocamos el cursor
         tft.setTextSize(2); // Especificamos el tamaño del texto
         tft.setTextColor(YELLOW); // Definimos el color del texto 
                  tft.println(temp); 

         tft.setCursor(150,170); // Colocamos el cursor
         tft.println(" Gr."); // Escribimos por pantalla
 ...
 tft.setCursor(90,190); // Colocamos el cursor
         tft.setTextSize(2); // Especificamos el tamaño del texto
         tft.setTextColor(YELLOW); // Definimos el color del texto 
         tft.println(hum); // Escribimos por pantalla

         tft.setCursor(150,190); // Colocamos el cursor
         tft.println(" %"); // Escribimos por pantalla
